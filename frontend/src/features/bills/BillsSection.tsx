...
import { format, addMonths, subMonths, isAfter, isBefore, addDays, endOfDay } from 'date-fns';

export const BillsSection = () => {
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const monthStr = format(currentMonth, 'yyyy-MM');

  const { bills, loading, fetchBills, updateBillStatus } = useBills();
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {bill.is_manual === 1 && <span title="Manual payment required"><HandMetal className="w-3 h-3 text-muted-foreground" /></span>}
